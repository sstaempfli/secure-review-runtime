# Authorized Attack Simulator (demo)
...
- Runtime API endpoints (the crawl `apiEndpoints` array, when present)
  that take object identifiers — candidate IDOR / auth-bypass targets.
...
`path_exposure`, `idor`, and `auth_bypass`. `path` must be a same-origin path.
Include `sourceFile` and `lineStart` when source context makes localization
clear.
